...
public class Solver5 : ISolver<Input, Output>
{
    public Output Solve(Input input)
    {
        ...
        foreach (var intersectionWithStreets in intersectionsWithStreets)
        {
            ...
            if (!validStreets.Any())
            {
                foreach (var street in intersectionWithStreets.Streets) schedule.Add(new StreetSchedule(street, 1));
            ...
            {
                var value = allCarPathsDict[street] / totalCrossings * 100;
            ...
            var sumVal = streetValue.Sum(c => c.Value);

            foreach (var street in validStreets)
            {
                var value = streetValue[street] / sumVal * maxPerCrossing;
